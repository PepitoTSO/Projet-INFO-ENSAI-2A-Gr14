...
from Service.SonService import SonService  # Assuming SonService is defined in a file named SonService.py
import pygame
...
class TestSonService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.son = Son(id_son=1, nom="Test Son", tags=["tag1"], path_stockage="data/test.mp3")
    ...
    @patch("pygame.mixer.music.get_busy", side_effect=[True, False])  # simulate end of playback after one loop
    def test_play(self, mock_get_busy, mock_play, mock_load):
        ...
        self.assertEqual(mock_get_busy.call_count, 2)  # Should be called twice: once busy, once not
    ...
    def test_play_channel(self, mock_channel, mock_sound):
        ...
        # Act
        self.son_service.play_channel(self.son)
        
        # Assert
        mock_sound.assert_called_once_with(self.son.path_stockage)  # Sound should be initialized with the path
        mock_channel.assert_called_once()  # Ensure we obtained a channel
        mock_channel_instance.play.assert_called_once_with(mock_sound_instance)  # play is called with the sound instance
    ...
    def test_jouer_en_boucle(self, mock_sound, mock_channel):
        ...
        mock_channel_instance.play.assert_called_once_with(mock_sound.return_value, -1, 500)

    @patch("random.uniform", return_value=1)  # Mocking the random.uniform to return 1 second for simplicity
    @patch("time.sleep", return_value=None)  # Mocking sleep to avoid actual waiting
    @patch.object(SonService, "play_channel")  # Mock play_channel to prevent real sound playing
    def test_jouer_aleatoire(self, mock_play_channel, mock_sleep, mock_uniform):
        # Act
        self.son_service.jouer_aleatoire(self.son, attente_min=1, attente_max=2, duree=3)
