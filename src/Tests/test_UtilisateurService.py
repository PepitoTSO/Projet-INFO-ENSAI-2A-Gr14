...
from Service.UtilisateurService import UtilisateurService  # Assurez-vous que le chemin d'import est correct
...
class TestUtilisateurService(unittest.TestCase):
    ...
    @patch.object(UtilisateurService, 'hacher_mot_de_passe')
    def test_creer_utilisateur_success(self, mock_hacher_mot_de_passe, mock_creer_utilisateur):
        mock_hacher_mot_de_passe.return_value = 'hashed_password'
        ...
        mock_hacher_mot_de_passe.assert_called_once_with(mdp_nh=mdp_nh)  # Ici, vous devez passer le nom de l'argument
        mock_creer_utilisateur.assert_called_once()

        # Vérifications de l'objet Utilisateur attendu
        utilisateur_attendu = Utilisateur(pseudo=pseudo_utilisateur, mdp_hache='hashed_password')
    ...
    @patch('Service.UtilisateurService.Utilisateur_DAO.creer_utilisateur')  # Patch sur la méthode correcte
    @patch.object(UtilisateurService, 'hacher_mot_de_passe')
    def test_creer_utilisateur_failure(self, mock_hacher_mot_de_passe, mock_creer_utilisateur):
        # Mock des retours
        mock_hacher_mot_de_passe.return_value = 'hashed_password'
        ...
        args, _ = mock_creer_utilisateur.call_args  # Obtenir les arguments passés lors de l'appel
    ...
    def test_deconnecter_utilisateur(self):
        Session.utilisateur = Utilisateur(pseudo='username', mdp_hache='hashed_password')
        self.service.deconnecter_utilisateur()
        self.assertIsNone(Session.utilisateur)

    @patch('Service.UtilisateurService.Utilisateur_DAO.modifier_utilisateur')
    @patch.object(UtilisateurService, 'hacher_mot_de_passe')
    def test_modifier_utilisateur_success(self, mock_hacher_mot_de_passe, mock_modifier_utilisateur):
        mock_hacher_mot_de_passe.return_value = 'new_hashed_password'
        mock_modifier_utilisateur.return_value = True

        Session.utilisateur = Utilisateur(pseudo='old_user', mdp_hache='old_hashed_password')
        pseudo_utilisateur = 'new_user'
    ...
    @patch('Service.UtilisateurService.Utilisateur_DAO.modifier_utilisateur')
    @patch.object(UtilisateurService, 'hacher_mot_de_passe')
    def test_modifier_utilisateur_failure(self, mock_hacher_mot_de_passe, mock_modifier_utilisateur):
        mock_hacher_mot_de_passe.return_value = 'new_hashed_password'
